...
def create_tables():
    ...
    try:
        mycursor.execute('''DROP TABLE IF EXISTS duplicate_products''')
        mycursor.execute('''CREATE TABLE duplicate_products(
            ID VARCHAR(255),
            name VARCHAR(255),
            price INT,
            brand TEXT,
    ...
    print("..... duplicate products table is successfully created")
...
def create_db():
    try:
        mycursor.execute('''DROP DATABASE IF EXISTS Elec_db''')
        mycursor.execute('''CREATE DATABASE Elec_db''')
        create_tables()
    except Exception as e:
        print(e)
